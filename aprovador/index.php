        ...
        <div class="menu" id="menuCelular">
            <div class="menuTop">
                ...
                <path d="M2.1532 17.5H16.1532" stroke="#0C4284" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    ...
                    <a href="../aprovador">Solicitações</a>
                </li>
            </div>
        ...
        <div class="lista" id="listaUsuario">
            ...
            <div class="filter-novo" id="trocaTelaB">
                <form action="" method="post" class="filter-novo-input">
                        ...
                        <input type="hidden"  name="" id="url" value="aprovador">
                ...
                <div class="filter-novo-add">
                    <button id="ir">+ &nbsp;Nova solicitação</button>
                </div>
            </div>
            ...
            <div class="lista-usuario"  id="trocaTelaC">
                ...
                if(empty($_POST['rodar'])){ $buscar_dados = mysqli_query($conexao, "select * from solicitacoes");
                                        
                                    
                }  if(isset($_POST['rodar'])){ $nome_cliente = $_POST['pesquisar'];
                    $buscar_dados = mysqli_query($conexao, "SELECT * from solicitacoes where solicitador like '$nome_cliente%'");

                }


                if(isset($_POST['aprovarMaterial'])){
                    $id = $_POST['id'];
                    $observacaoAtualiza = $_POST['observacao'];

                    $inserir = mysqli_query($conexao, "UPDATE solicitacoes
                    SET status_solicitacao = 'aprovado' , observacao = '$observacaoAtualiza'
                    WHERE id = '$id'");

                    echo "<script> alert('MATERIAL APROVADO'); location.href = '../aprovador'; </script>";
                ...
                if(isset($_POST['reprovarMaterial'])){
                    $id = $_POST['id'];
                    $observacaoAtualiza = $_POST['observacao'];

                    $inserir = mysqli_query($conexao, "UPDATE solicitacoes
                    SET status_solicitacao = 'reprovado', observacao = '$observacaoAtualiza'
                    WHERE id = '$id'");

                    echo "<script> alert('MATERIAL REPROVADO'); location.href = '../aprovador'; </script>";
                ...
                $observacao = $row_buscar['observacao'];
                    ...
                    echo "
                         ...
                         <ul> 
                            ...
                            echo"
                            <li> <button class='botao-icone' onclick='visualizarSolicitacao($c)'> <svg width='22' height='14' viewBox='0 0 22 14' fill='none' xmlns='http://www.w3.org/2000/svg'>
